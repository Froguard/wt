...
function simplifyData(data){
    ...
    res.list = data.map((item) => {
        ...
        return {
            ...
            wz_num: item.wz_num,
            qt_num: item.qt_num,
            controlled_train_message: item.controlled_train_flag === '1' ? item.controlled_train_message : (!canWebBuy? CANT_BUY : "" )
        };
    });
...
function printOut(data){
    ...
    notify({
        ...
        message: `${res.from} => ${res.to} ${res.hasTicket?"有票啦！":"没有票！"}`,
...
function doQuery(config,uid){
    ...
    promise.then(
        (res) => {
            ...
            console.log(`\r\n${getCurT()} 监视器 ${colors.magenta(uid)} 成功查询到 ${colors.cyan(res.length)} 条数据！ ${info}\r\n`);
            printOut(res);
        },
        (err) => {
            console.log(err);
...
Watcher.prototype = {
    ...
    start: function(){
        ...
        let info = colors.gray("("+chineseInfo.from+"->"+chineseInfo.to+","+chineseInfo.date+","+chineseInfo.onlyShowHighSpeedRail+chineseInfo.purposeCode+")");
        console.log(`${getCurT()} 开启监视器 ${colors.magenta(self.uid)} ${info}`);
